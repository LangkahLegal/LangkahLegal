...
def posting_kasus_anonim(
    ...
    if current_user.get("role") != "client":
        raise HTTPException(status_code=403, detail="Hanya klien yang dapat memposting kasus")
...
def lihat_bursa_kasus(
    ...
    if current_user.get("role") != "konsultan":
        raise HTTPException(status_code=403, detail="Hanya konsultan yang bisa melihat bursa")

    response = db.table("bursa_kasus").select("*").eq("status_bursa", "open").execute()
    return {
        "message": "Berhasil mengambil data bursa kasus",
        "data": response.data
    }
...
def kirim_penawaran_bidding(
    ...
    if current_user.get("role") != "konsultan":
        raise HTTPException(status_code=403, detail="Hanya konsultan yang bisa mengirim bid")

    # 1. CARI ID_KONSULTAN YANG ASLI DARI TABEL KONSULTAN
    konsultan_profile = db.table("konsultan").select("id_konsultan").eq("id_user", current_user["id_user"]).execute()
    
    if not konsultan_profile.data:
        raise HTTPException(status_code=404, detail="Profil konsultan tidak ditemukan. Pastikan Anda sudah terdaftar sebagai konsultan.")
    
    real_id_konsultan = konsultan_profile.data[0]["id_konsultan"]

    # 2. Cek apakah status bursa masih open
    kasus = db.table("bursa_kasus").select("status_bursa").eq("id_bursa", id_bursa).execute()
    if not kasus.data or kasus.data[0]["status_bursa"] != "open":
        raise HTTPException(status_code=400, detail="Kasus tidak ditemukan atau bursa sudah ditutup")
...
def terima_penawaran(
    ...
    bid = db.table("penawaran_konsultan").select("*").eq("id_penawaran", id_penawaran).execute()
    if not bid.data:
    ...
    db.table("bursa_kasus").update({"status_bursa": "closed"}).eq("id_bursa", bid_data["id_bursa"]).execute()
    db.table("penawaran_konsultan").update({"status_penawaran": "diterima"}).eq("id_penawaran", id_penawaran).execute()

   # Saat Client klik 'Accept'
    new_consultation = {
    "id_user": current_user["id_user"],
    "id_konsultan": id_konsultan_pemenang,
    "id_bursa": id_bursa_terkait,      # Data dari tabel bursa_kasus
    ...
    return {"message": "Penawaran diterima, draf konsultasi dibuat", "data": response.data[0]}
...
def lihat_penawaran_masuk(
    ...
    response = db.table("penawaran_konsultan").select("*, konsultan(nama_lengkap, spesialisasi)")\
        .eq("id_bursa", id_bursa).execute()
