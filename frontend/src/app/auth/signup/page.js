          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isConsultant && (
              <>
                ...
                <div className="form-field">
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {SPECIALIZATIONS_LIST.map((spec) => {
                      ...
                      return (
                        <button key={spec} type="button" onClick={() => handleSpecializationToggle(spec)} className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 border ${isSelected ? "bg-[#6f59fe] text-white border-[#6f59fe] shadow-[0_0_15px_rgba(111,89,254,0.4)] pr-3" : "bg-[#131125] text-[#aca8c1] border-[#2a2840] hover:border-[#ada3ff]/50"}`}>
                          <span>{spec}</span>
                          {isSelected && <MaterialIcon name="close" className="text-[16px] opacity-80 hover:opacity-100 transition-opacity" />}
                        </button>
                      );
                    })}
                  </div>
                ...
                <InputField label="Pengalaman Praktik (Tahun)" name="experience" type="number" placeholder="3" value={formData.experience} onChange={handleChange} required />
