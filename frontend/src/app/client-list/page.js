...
export default function ConsultantClientsPage() {
  ...
  const sortedClients = [...clients].sort((a, b) => {
    const dateA = new Date(a.consultationDateRaw).getTime();
    const dateB = new Date(b.consultationDateRaw).getTime();
          ...
          <div className="max-w-4xl mx-auto w-full space-y-8">
            ...
            <div className="flex justify-between items-end px-2">
              ...
              <div className="relative">
                <Filter 
                  isActive={isFilterOpen || sortOrder === "desc"} 
                  ...
                  <div className="absolute right-0 mt-3 w-64 bg-[#1f1d35] rounded-2xl border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-20 py-2 overflow-hidden flex flex-col">
                    ...
                    <div className="px-4 py-2">
                      ...
                      <button 
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg mb-1 ${sortOrder === "asc" ? "bg-[#6f59fe] text-white font-bold" : "text-[#aca8c1] hover:bg-white/5 hover:text-white"}`}
                      >
                        <div className="flex items-center gap-2">
                          <MaterialIcon name="arrow_downward" className="text-[14px]" />
                          <span>Terdekat (Default)</span>
                        </div>
                        {sortOrder === "asc" && <MaterialIcon name="check" className="text-[14px]" />}
                      </button>

                      {/* Paling Jauh */}
                      <button 
                        onClick={() => handleSortChange("desc")} 
                        className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg ${sortOrder === "desc" ? "bg-[#6f59fe] text-white font-bold" : "text-[#aca8c1] hover:bg-white/5 hover:text-white"}`}
                      >
                        <div className="flex items-center gap-2">
                          <MaterialIcon name="arrow_upward" className="text-[14px]" />
                          <span>Terjauh</span>
                        </div>
                        {sortOrder === "desc" && <MaterialIcon name="check" className="text-[14px]" />}
                      </button>
